...
public record Sphere(double radius, Color color, Point center) {
    ...
    public Hit intersect(Ray r) {
        ...
        if (0 <= discriminant) {
            ...
            if (r.isValid(t1) && t1 < t2) {
                ...
                return new Hit(t1, r.pointAt(t1), Vector.normalize(n), color);
            }

            if (r.isValid(t2) && t2 < t1) {
                Direction n = Vector.divide(Vector.subtract(r.pointAt(t2), center), radius);
                return new Hit(t2, r.pointAt(t2), Vector.normalize(n), color);
            }
        }
